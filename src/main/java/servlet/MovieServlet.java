...
public class MovieServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        String pathInfo = req.getPathInfo(); // 예: /top100, /genres, /movie/123
        if (pathInfo == null) {
    ...
    // ✅ 영화 상세: /api/movie/{id}
    private void handleMovieDetail(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        String pathInfo = req.getPathInfo(); // /movie/123
        String id = pathInfo.substring("/movie/".length());

        String lang = getOrDefault(req.getParameter("lang"), "ko-KR");

        String url = TMDB_BASE + "/movie/" + encode(id)
